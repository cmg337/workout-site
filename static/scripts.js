//groups used in SQL
let muscle_groups = ["chest", "biceps", "triceps", "back", "legs", "shoulders", "traps", "abs"];
let workout_types = ["body", "weight", "machine"];
...
$(document).ready(function(){
    ...
    $( "#workout_form" ).on('submit', function(event) {
        ...
        for(i in muscle_groups){
            if($( "#" + muscle_groups[i] ).prop("checked")){
                group_check = true;
                break;
            };
        };
        ...
        for(i in workout_types){
            if($( "#" + workout_types[i] ).prop("checked")){
                type_check = true;
                break;
            };
        };

        if(!group_check || !type_check){
            event.preventDefault();
            $( '#alert' ).addClass('alert alert-danger');
            $( '#alert' ).html('At least 1 group and 1 type required');
            return false;
        }
    });
...
function saveWorkout(workout){
    ...
    var workoutJSON = {'length': workout.length};
